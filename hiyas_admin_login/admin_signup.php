<?php
    ...
    function generateUniqueUserId($mysqli) {
        do {
            ...
            $stmt = $mysqli->prepare("SELECT user_id FROM employee WHERE user_id = ?");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['email'], $_POST['employee_username'], $_POST['password'])) {
            // Signup request
            $email = $_POST['email'];
            ...
            if ($result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("SELECT employee_username FROM employee WHERE employee_username = ?");
                ...
                if ($result->num_rows > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO employee (user_id, email, employee_username, password, date) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssss", $user_id, $email, $employee_username, $password, $date);
                    if($stmt->execute()) {
